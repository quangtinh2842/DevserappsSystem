...
//    guard let _ = (scene as? UIWindowScene) else { return }
        ...
        MUser.find(byId: currentUser!.uid) { [weak self] result, error in
          ...
          if error != nil {
            ...
            if result != nil {
              ...
              Task {
                do {
                  try await self?.showMainTBCInCaseAfterLaunch()
                } catch {
                  self?.window?.rootViewController = StoryboardHelper.newHardToHandleErrorVC()
                }
                KRProgressHUD.dismiss()
  ...
  func showMainTBCInCaseAfterLaunch() async throws {
    try await _mainTBC.doSomethingAfterSignIn()
    window?.rootViewController = _mainTBC
  }
  
  func showMainTBCInCaseAfterSignIn() async throws {
