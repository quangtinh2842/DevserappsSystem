...
class EmailVerificationViewController: UITableViewController {
  ...
  private func _didVerifyEmailSuccessfullyHandler(withUID uid: String) {
    ...
    MUser.find(byId: uid) { [weak self] result, error in
      ...
      if let syncedUser = result as? MUser {
        ...
        Task {
          do {
            try await self?._presentMainVCInCaseAfterSignIn()
          } catch {
            self?.view.makeToast("Error: "+error.localizedDescription, duration: 3.0, position: .bottom)
          }
          KRProgressHUD.dismiss()
        }
        return
  ...
  private func _presentMainVCInCaseAfterSignIn() async throws {
    let scene = UIApplication.shared.connectedScenes.first as! UIWindowScene
    let sceneDelegate = scene.delegate as! SceneDelegate
    try await sceneDelegate.showMainTBCInCaseAfterSignIn()
  }
}
